  ...
  return (
    <div className="p-6 lg:p-8 space-y-6 max-w-5xl">
      ...
      <div className="relative">
        ...
        <input type="search" placeholder="Search experiences…" value={search} onChange={(e) => setSearch(e.target.value)} className="w-full rounded-lg border border-slate-700 bg-slate-800/60 pl-9 pr-3 py-2.5 text-sm text-white placeholder-slate-500 focus:border-amber-500/80 focus:outline-none" />
      </div>
      ...
      {loading ? (
        ...
        <div className="rounded-xl border border-slate-700/60 overflow-hidden">
          ...
          {filtered.map((x, i) => (
            <div key={x.slug} className={`grid grid-cols-[1fr_2fr_auto_auto_auto] items-center gap-4 px-4 py-3 border-t border-slate-700/40 hover:bg-slate-800/30 transition-colors ${i === 0 ? "border-t-0" : ""}`}>
              <div className="h-11 w-16 shrink-0 rounded-md overflow-hidden bg-slate-900">
                {(x.imageUrl ?? x.heroImage?.url) ? (
                  ...
                  <div className="flex h-full items-center justify-center">
                    <svg className="h-5 w-5 text-slate-700" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16" /></svg>
                  </div>
                )}
              </div>
              <div>
                <p className="text-sm font-medium text-white">{x.name}</p>
                <p className="text-xs text-slate-500">/experiences/{x.slug}</p>
              </div>
              <div>
                {x.durationDays ? (
